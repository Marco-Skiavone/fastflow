...
void manager(ff_pipeline& pipe, size_t n_threads, size_t period_deadline) {
	...
	while(!managerstop && times < n_memory_records) {   // and memory has space
        ...
        // We remove from the one (decr_i)
        // and giving to the one having the min (negative - it has greater input queue than output one)
        if (!managerstop && decr_i != min_i && min_i != SIZE_MAX && rt_table[decr_i] >= runtime_min && rt_table[min_i] <= runtime_max) {
            ...
            if (set_deadline_attr(n_threads, period_deadline, rt_table[decr_i], node_tids[decr_i])) {
            	// error case: returning to previous values
                rt_table[decr_i] += runtime_offset;
                rt_table[min_i] -= runtime_offset;
...
int main(int argc, char* argv[]) {
    ...
    std::ofstream oFile(REL_PATH + "times3.csv", std::ios_base::app);
    if (oFile.is_open()) {
        if (oFile.good()) {
            oFile << ntasks << "," << nnodes << "," << period_deadline << "," << RUNTIME_FRACTION << "," << tot_time << std::endl;
            std::cout << "- Time saved on times3.csv" << std::endl;
        } else {
